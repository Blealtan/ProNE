...
float bessel(int a, float b) { return boost::math::cyl_bessel_i(a, b); }
...
MatrixXf randomizedRangeFinder(SMatrixXf &A, int size, int num_iter) {
  ...
  for (int i = 0; i < num_iter; i++) {
    lu1.compute(A * Q);
    L.setIdentity();
    L.block(0, 0, n_samples, size).triangularView<Eigen::StrictlyLower>() =
        lu1.matrixLU();
    L = lu1.permutationP().inverse() * L;

    lu2.compute(A.transpose() * L);
    Q.setIdentity();
    Q.block(0, 0, n_features, size).triangularView<Eigen::StrictlyLower>() =
        lu2.matrixLU();
...
MatrixXf randomizedSvd(SMatrixXf &data, int rank, int num_iter) {
  ...
  // Eigen::JacobiSVD<MatrixXf> svdOfB(B, Eigen::ComputeThinU |
  // Eigen::ComputeThinV);
...
MatrixXf getEmbbeddingViaSvd(SMatrixXf &data, int rank) {
  ...
  MatrixXf emb =
      redsvd.matrixU() * redsvd.singularValues().cwiseSqrt().asDiagonal();
  emb = l2Normalize(emb);
  return emb;
}

MatrixXf getEmbbeddingViaDenseSvd(MatrixXf &data, int rank) {
  // Eigen::JacobiSVD<Eigen::MatrixXf> svdOfC(data, Eigen::ComputeThinU |
  // Eigen::ComputeThinV); Eigen::BDCSVD<Eigen::MatrixXf> svdOfC(data,
  // Eigen::ComputeThinU | Eigen::ComputeThinV);
  Eigen::BDCSVD<Eigen::MatrixXf> svdOfC(data, Eigen::ComputeThinU);
  MatrixXf emb =
      svdOfC.matrixU() * svdOfC.singularValues().cwiseSqrt().asDiagonal();
  emb = l2Normalize(emb);
  return emb;
}
...
MatrixXf getSpectralEmbedding(SMatrixXf &A, MatrixXf &a, int step, float theta,
                              float mu) {
  time_t t1 = time(NULL);
...
int main(int argc, char **argv) {
  ...
  MatrixXf embedding =
      getSpectralEmbedding(A, feature, FLAGS_num_step, FLAGS_theta, FLAGS_mu);
  time_t t3 = time(NULL);
